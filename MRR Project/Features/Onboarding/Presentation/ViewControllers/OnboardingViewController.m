...
static void MRROnboardingRecipeFlowLog(NSString *format, ...) {
  va_list arguments;
  va_start(arguments, format);
  NSString *message = [[[NSString alloc] initWithFormat:format arguments:arguments] autorelease];
  va_end(arguments);
  NSLog(@"[OnboardingRecipeFlow] %@", message);
}
#else
static void MRROnboardingRecipeFlowLog(__unused NSString *format, ...) {}
#endif
...
- (NSArray<OnboardingRecipePreview *> *)loadRecipes {
  return [self.recipeCatalog allRecipePreviews];
}
...
- (void)presentRecipeDetailViewController:(OnboardingRecipeDetailViewController *)detailViewController {
  ...
  MRROnboardingRecipeFlowLog(@"Presenting recipe detail view controller. loading=%@.", detailViewController.isLoading ? @"YES" : @"NO");
...
- (void)presentLoadedRecipeDetail:(OnboardingRecipeDetail *)detail
                    recipePreview:(OnboardingRecipePreview *)recipePreview
                      debugOrigin:(OnboardingRecipeDetailDebugOrigin)debugOrigin {
  if (detail == nil || recipePreview == nil) {
    return;
  }

  OnboardingRecipeDetailViewController *detailViewController =
      [[[OnboardingRecipeDetailViewController alloc] initWithRecipePreview:recipePreview recipeDetail:detail] autorelease];
  detailViewController.debugOrigin = debugOrigin;
